...
线下结果按 `row_name + text_index` 合并为 UTF-8 的 `analysis_ja.tsv`。运行时第一次按解析键时延迟加载，并以当前 `LineRecord.VoiceLabel + TextIndex` 精确查询；未完成解析的台词显示暂无解析，不会回退到同一行的其他文本槽位。游戏运行过程中没有模型调用或网络请求。
